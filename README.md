...
* 自己封装了由参数绘制海报的函数，减小了大量API的重复编写，提高了开发效率
...
* 方案二：理论上，该方案是最优的解决方案，node-canvas的registerFont方法支持字体的引入，失败原因是：寻找不到生效并且完全匹配的emoji.ttf文件，自己手动匹配创建一个支持emoji的字体文件成本过大，工作量极大，需要匹配2000多个文件。并且ttf文件的生成更是一个大坑。（理论可行，事实碰壁）  
...
#### 参与开发的页面 ： 电商搜索页、品牌页、帖子审核系统

### 收获
* 拥有了移动端的开发经验，对CSS有了更高的追求和更深的理解
* 遇见了一些移动端滑动事件、dom操作、动画并能顺利解决，能适应更加复杂的业务场景的开发
* 对服务端渲染有了更深入的理解。

#### 服务端渲染和本地浏览器在开发维度上的区别和思考
* 服务端是没有window对象
* 服务端渲染和本地环境生命周期钩子执行的时期不一样，谨慎处理在create（）生命周期里的数据
